...
export default function SkillsSection() {
  ...
  return (
    <section
      id="skills"
      ref={sectionRef}
      className="relative min-h-screen bg-black py-32 px-6 md:px-12 flex flex-col justify-center"
    >
      <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row gap-16 lg:gap-8 items-center">
        ...
        <div className="w-full lg:w-1/2 space-y-12 z-10">
          <div>
            ...
            <p className="font-mono text-sm tracking-widest text-gray-400 uppercase max-w-md leading-relaxed">
          ...
          <div className="flex flex-col gap-4">
            {skillCategories.map((category) => (
              <div
                ...
                className={`group cursor-pointer border-b border-gray-800 pb-4 transition-colors duration-300 ${activeCategory.id === category.id ? 'border-lando-lime' : 'hover:border-gray-500'}`}
              >
                <div className="flex items-center justify-between">
                  <h3 className={`font-display text-3xl uppercase transition-colors duration-300 ${activeCategory.id === category.id ? 'text-lando-lime' : 'text-gray-500 group-hover:text-white'}`}>
                    {category.title}
                  </h3>
                  <div className={`w-3 h-3 rounded-full transition-colors duration-300 ${activeCategory.id === category.id ? 'bg-lando-lime' : 'bg-transparent'}`} />
                </div>

                {/* Mobile view of skills directly under */}
                <div className={`lg:hidden mt-4 transition-all duration-300 overflow-hidden ${activeCategory.id === category.id ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                  <div className="flex flex-wrap gap-2">
                    {category.skills.map(skill => (
                      <span key={skill} className="px-2 py-1 bg-white/10 text-white text-xs font-mono uppercase">
                        {skill}
                      </span>
                    ))}
                  </div>
